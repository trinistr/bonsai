  ...
  describe "generator" do
    ...
    it "should create base directories" do
      %w(content content/index templates public public/css public/js).each do |dir|
        File.directory?("#{@path}/#{dir}").should be true
      end
    end
    
    it "should copy the htaccess file to public/.htaccess" do
      File.exists?("#{@path}/public/.htaccess").should be true
    end
    
    it "should copy the base.scss file" do
      File.exists?("#{@path}/public/css/base.scss").should be true
    end
    
    it "should copy an index page" do
      File.exists?("#{@path}/content/index/default.yml").should be true
    end
    
    it "should copy a default template" do
      File.exists?("#{@path}/templates/default.liquid").should be true
    end
    
    it "should create a robots.txt" do
      File.exists?("#{@path}/public/robots.txt").should be true
    end
    
    it "should create a site.yml" do
      File.exists?("#{@path}/site.yml").should be true
    end
